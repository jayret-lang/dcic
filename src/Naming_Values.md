...
::: {.do-now}
Try typing `puppy`{.pyret} at the interactions pane prompt (›››). Are there any terms in the
...
now, remember that the names you associate with values using `=`{.pyret}
...
::: {.exercise}
Assuming you still have the `blue-circ`{.pyret} definition from above in
...
Since `blue-circ`{.pyret} yielded a result, we infer that a name by
...
`purple-tri`{.pyret} twice in the same expression without having to write
out the longer `triangle`{.pyret} expression more than once.

::: {.exercise}
Assume that your definitions pane contained only this most recent
code example (including the `purple-tri`{.pyret} and `green-sqr`{.pyret} definitions). How many separate images
