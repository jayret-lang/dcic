...
`is`{.jayret}, we are implicitly making a statement about equality between
...
```
Observe that we use `b!v`{.jayret} to extract the current value, and use
the naming convention of `-now`{.jayret} to make clear these are stateful
...
```
However, we cannot write `set-box-now(n1, "hi")`{.jayret}, because that
would violate the type of `n1`{.jayret}, which is `Box < Number >`{.jayret}. We
could make this explicit by writing

```jayret
n1 = box(1);
```
if we wanted to be explicit. However, note that `n1`{.jayret} being a box
...
extracted from `n1`{.jayret}—there is no danger that it will suddenly
...
```
Observe that `b1`{.jayret} and `b3`{.jayret} are referring to the same
box, while `b2`{.jayret} is referring to a different one. We can see this
...
```
In other words, `b1`{.jayret} and `b3`{.jayret} are aliases for the same box,
but neither is an alias to the box referred to by `b2`{.jayret}. Since
`identical`{.jayret} is transitive, it follows from the first two
...
It’s unsurprising that the first test, `assertEquals(b1, b3)`{.jayret},
passes. However, the second, `assertEquals(b1, b2)`{.jayret}, fails! And the name
...
`b1`{.jayret} and `b3`{.jayret} are aliases, their values change in lock-step
...
[[<three-boxes>](state-in-pyret.html#%28elem._three-boxes%29)] but not mutated `b1`{.jayret}. That is, when
printed, all three boxes have the same value, `box(7)`{.jayret}. We
have seen that `b1`{.jayret} and `b3`{.jayret} are both
`equal-always`{.jayret} and `identical`{.jayret} to each other. However, we
have also seen that `b1`{.jayret} and `b2`{.jayret} are neither of
...
```
The `-now`{.jayret} in the name reminds us that these values are equal at
the moment, but may not be equal later. Sure enough, if we add

```jayret
set-box-now(b1, 8);
```
back into the program, the above `equal-now`{.jayret} tests fail: now,
they are no longer equal!

Recall that the other two equality predicates have an binary operator
notation: `==`{.jayret} for `equal-always`{.jayret} and `<=>`{.jayret} for
`identical`{.jayret}. Similarly, `equal-now`{.jayret} has the binary operator
`=~`{.jayret}. You should view that as `=`{.jayret} with hand-waving `~`{.jayret}:
...
whether `b1`{.jayret}, `b2`{.jayret}, or `b3`{.jayret} has had its content modified.
...
Observe that if two expressions are `identical`{.jayret}, then they are
aliases, i.e., they are referring to one and the same
value. Therefore, the values produced by those expressions must be
`equal-always`{.jayret}. If they are always equal, then clearly at any
given moment, they must also be `equal-now`{.jayret}.

Even if two expressions are not `identical`{.jayret}, they may be
`equal-always`{.jayret}. This would never be true of mutable data (because
...
If two data are `equal-now`{.jayret}, they may not be
`equal-always`{.jayret}: if they are mutable, a future mutation may change
the equality, as we have seen above. Similarly, two data may be
`equal-always`{.jayret} but not be `identical`{.jayret}, because they reside
...
corresponding to `identical`{.jayret} (known as reference equality)
and `equal-now`{.jayret} (known as structural equality). Jayret is
rare in having a third operator, `equal-always`{.jayret}. For
...
assumptions). This is why `is`{.jayret} in testing uses
`equal-always`{.jayret} by default, and forces users to explicitly pick a
...
`identical`{.jayret} always takes constant time. Indeed, some programs use
`identical`{.jayret} precisely because they want constant-time
...
`equal-always`{.jayret} and `equal-now`{.jayret} both must traverse at least
...
data). The difference is that `equal-always`{.jayret} reduces to
`identical`{.jayret} at references, thereby performing less computation
than `equal-now`{.jayret} would.
...
```
Observe that `b1-id-b2`{.jayret} would be `false`{.jayret} but `b1-id-b3`{.jayret}
would be `true`{.jayret}. And notice that this would always be true when
...
In practice, `identical`{.jayret} does not behave this way: it would be
...
`identical`{.jayret}: two values are `identical`{.jayret} precisely when, when
...
definition of `identical`{.jayret}, it makes sense; if other operators do
...
you will get the answer `false`{.jayret}.) This ensures that the
...
version of each of the above three operators (`identical3`{.jayret},
`equal-always3`{.jayret} and `equal-now3`{.jayret}), all of which return
`EqualityResult`{.jayret} values that correspond to truth, falsity, and
